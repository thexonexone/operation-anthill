...
| Approval-gated actions | V2.3.0–V2.3.1 | **SHIPPED** | ActionProposal pipeline (propose → blast-radius score → approve → execute → verify → audit), allowlisted action catalog with structural forbidden set, Local + Mock runners, HOMELAB_STOP kill switch + `/homelab/actions/stop`/`resume`, unified-queue projection, Actions console panel; v2.3.1 added the ProxmoxActionRunner (double-gated `homelab_proxmox_write_actions_enabled`, POST shapes structurally allowlisted, clean-shutdown-only, verified execution); v2.3.2 adds D1 target-guard enforcement + node-segment validation and the Service Deck console redesign. Action capability gates still ship OFF — fail closed |
