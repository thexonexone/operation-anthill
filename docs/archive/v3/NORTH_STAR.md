...
**Baseline:** v2.26.0 · **Shipping release: v3.8.23** (patches are verified in a tree that CONTAINS them — v3.8.22 pointed the build verifier at the primary workspace, so every verdict was a true statement about the repository as it already was and a non-compiling proposal passed every time; `PatchSetMaterializer` writes the whole set into a disposable copy and verification runs there, failing as a unit and recording a `workspace_snapshot`. All twelve roles now have execution contracts — the six that lacked one were the core ants including the coder, hidden because the missing-contract check ran only `if (isSpecialist)` — with `SchedulingMode` declared and the three phantom tools removed from contracts rather than built; preceded by v3.8.22) (a correction release: v3.8.21 reported patches verified and they were not — the planner emits `patch_proposal`, the policy table is keyed `code_patch`, nothing mapped them, so `diff` and `build` never ran on a single patch and a proposal that does not compile could reach `completed_verified`; the request also carried no patch content, and the promotable verdict and the soldier’s deterministic policy block were both computed and read by nothing — both now demote the mission outcome; preceded by v3.8.21) (patches are verified for the first time: `VerificationRunner` gets its first production call site since v2.12, so a patch that does not compile can no longer reach a verified outcome — with the default policy narrowed to drop the full-suite verifier, because requiring it meant half an hour per patch task and a suite that could invoke itself; three core ants also start emitting typed artifacts and three deliberately do not; preceded by v3.8.20) (the artifact and evidence stores get producers: ants' already-typed artifacts become first-class rows instead of a JSON blob, and deterministic evidence is recorded at the tool chokepoint because `VerificationRunner` — which owns the deterministic verifiers — turned out to have no production call site at all; preceded by v3.8.19) (the first post-refactor release: ADR-004's artifact and evidence stores land additively, pheromone trails decay for the first time, and colony memory gets the four retrieval questions it was supposed to answer; reputation and typed pheromones deliberately wait for artifacts, because until ants stop passing prose that is what they would learn from; preceded by v3.8.18) (refactor sign-off: an external review found six acceptance gaps in v3.8.17 and all six were valid — injected tool policy was bypassed at execution, `SafetyPolicy` was publicly mutable, and the no-UI gate proved a null check rather than a build; see `docs/REFACTOR-PLAN.md` §7; preceded by v3.8.17) (the Core/Modules refactor ENDS: phases 6 and 7 ship, `ApiHost.cs` goes from 3,294 lines to 535 across six partials, the console assets move to `Anthill.UI`, `py.old/` is deleted, and five of six success criteria are met — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.16) (phase 5 ends: the six tools that act on the machine move to `Anthill.Modules.Tools` while registration, authorization and dispatch stay in the core, taking it to 24,973 lines from a 34,247 baseline; ADR-007 records the boundary; preceded by v3.8.15) (phase 5c step 3: `ToolDefinition` and `IToolKindExecutor` join `Anthill.SDK.Tools`, with the three checks that read the core's own tables now asking it through `IToolDefinitionPolicy`; preceded by v3.8.14) (phase 5c step 2 completes: `TextUtil` joins `Anthill.SDK.Common`, 119 of its 121 references untouched, with its one mutable setting joining `IToolRuntimeOptions`; preceded by v3.8.13) (a model-supplied patch filename could dispatch a second UI action in the operator's session; the console's executable attributes no longer carry untrusted values, which now travel in plain data-* attributes read through a fixed action map; preceded by v3.8.12) (phase 5c step 2, first half: the SSRF and patch-path guards move to `Anthill.SDK.Common` behind live-reading options, with every one of their 21 call sites unchanged; preceded by v3.8.11) (phase 5c step 1: the tool capability gates move behind a live-reading contract, preserving the colony's two-level gating; preceded by v3.8.10) (phase 5b: the tool contract joins the SDK and modules can finally contribute tools; preceded by v3.8.9) (phase 5a: the shared half of the contract vocabulary joins the SDK, with the Task-coupled half deliberately left in the core; preceded by v3.8.8) (the Core/Modules boundary is enforced by assembly-reference tests rather than by review; preceded by v3.8.7, the homelab leaving the core) (phase 4a: `AnthillTime` and `Json` move to `Anthill.SDK.Common` — the only two Common helpers Homelab and Integrations actually use — and homelab events reach the colony's live stream for the first time; see `docs/REFACTOR-PLAN.md`; preceded by v3.8.6) (the module lifecycle gets a caller: `ModuleHost` loads modules through `IModuleContext`, and `SqliteMemory` implements the SDK's `IPheromoneMemory` and `IEventLog` so a module holds two narrow views rather than the whole store — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.5) (the colony runs without AI: provider construction inverted behind `IReasoningProviderFactory`, implementations extracted to `Anthill.Modules.Reasoning`, and nothing in `Anthill.Core` naming a provider type — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.4) (the reasoning protocol and `IReasoningProvider` move to `Anthill.SDK.Reasoning`, so the provider contract no longer lives in the core that is required to run without providers — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.3) (the Core/Modules refactor begins: `Anthill.SDK` as a contracts-only project, and a live event bus retrofitted behind `SqliteMemory.LogEvent` without touching one of its ~85 call sites — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.2) (the agent harness: a typed provider substrate, a bounded tool-calling loop, operator-defined tools, and contracts that declare what they need from a model — v3.3.0 carried the provider substrate, v3.4.0 the tool framework, v3.4.1 user-defined tools; v3.2.1 was the last dashboard release)
