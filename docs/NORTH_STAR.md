...
**Baseline:** v2.26.0 · **Shipping release: v3.8.3** (the Core/Modules refactor begins: `Anthill.SDK` as a contracts-only project, and a live event bus retrofitted behind `SqliteMemory.LogEvent` without touching one of its ~85 call sites — see `docs/REFACTOR-PLAN.md`; preceded by v3.8.2) (the agent harness: a typed provider substrate, a bounded tool-calling loop, operator-defined tools, and contracts that declare what they need from a model — v3.3.0 carried the provider substrate, v3.4.0 the tool framework, v3.4.1 user-defined tools; v3.2.1 was the last dashboard release)
