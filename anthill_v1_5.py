# ============================================================
#  ANTHILL CORE v1.5 - Single File Python Version
#  Requirements: pip install pydantic rich requests fastapi uvicorn
#  Optional local AI: ollama pull llama3.1:8b
#
#  v1.5 focus:
#  - Local UI Foundation
#  - Browser dashboard served from /ui
#  - Mission submit, job monitor, event stream, tasks, approvals, patches, sources, pheromones
#  - Preserves v1.4.2 API runtime safety and all backend gates
# ============================================================
...
API_MAX_LIMIT = 200
SYSTEM_API_MISSION_ID = "system_api"
API_JOB_MAX_HISTORY = 100
API_JOB_WORKERS = 1
API_PERMISSIONS = {
    "run_mission": True,
    "approve": True,
...
class ModelRouter:
    ...
    def format_routes(self) -> str:
        lines = ["ANTHILL v1.4.2 Model Routes"]
...
class SQLiteMemory:
    ...
    def get_tasks_for_mission(self, mission_id: str, limit: int = 200) -> List[Dict[str, Any]]:
        with self._connect() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("""
                SELECT id, mission_id, title, description, assigned_ant, task_type,
                       status, result, result_summary, result_chars, estimated_tokens,
                       started_at, finished_at, elapsed_seconds
                FROM tasks
                WHERE mission_id = ?
                ORDER BY COALESCE(started_at, finished_at, id) ASC
                LIMIT ?
            """, (mission_id, limit))
            return [dict(row) for row in cursor.fetchall()]
...
class Queen:
    ...
    def format_approvals(self, limit: int = 20, status: Optional[ApprovalStatus] = ApprovalStatus.PENDING) -> str:
        rows = self.memory.list_approval_requests(status=status, limit=limit)
        status_label = status.value if isinstance(status, ApprovalStatus) else "all"
        if not rows:
            return f"No approval requests found for status: {status_label}."
        blocks = []
        for approval in rows:
            created = approval.get("created_at") or "n/a"
            decided = approval.get("decided_at") or "n/a"
            description = truncate_text(approval.get("description") or "", 260, suffix="...[description truncated]")
            blocks.append(
                f"Approval ID: {approval.get('id')}\n"
                f"Status: {approval.get('status')}\n"
                f"Action: {approval.get('action_type')}\n"
                f"Target ID: {approval.get('target_id')}\n"
                f"Mission ID: {approval.get('mission_id')}\n"
                f"Task ID: {approval.get('task_id') or 'n/a'}\n"
                f"Title: {approval.get('title')}\n"
                f"Requested By: {approval.get('requested_by')}\n"
                f"Created At: {created}\n"
                f"Decided At: {decided}\n"
                f"Decision Note: {approval.get('decision_note') or 'n/a'}\n"
                f"Description:\n{description}"
            )
        return f"Approval Requests | status={status_label} | count={len(rows)}\n\n" + ("\n" + "-" * 50 + "\n").join(blocks)

    def format_mission_detail(self, mission_id: str) -> str:
        mission = self.memory.get_mission(mission_id)
        if not mission:
            return f"No mission found with id: {mission_id}"
        tasks = self.memory.get_tasks_for_mission(mission_id)
        goal = truncate_text(mission.get("goal") or "", 600, suffix="...[goal truncated]")
        user_result = truncate_text(mission.get("user_result") or mission.get("final_result") or "", 1200, suffix="...[result truncated]")
        task_lines = []
        for task in tasks:
            summary = truncate_text(task.get("result_summary") or task.get("result") or "", 260, suffix="...[summary truncated]")
            task_lines.append(
                f"- {task.get('id')} | {task.get('assigned_ant')} | {task.get('task_type')} | "
                f"{task.get('status')} | {task.get('elapsed_seconds') or 0}s | {task.get('title')}\n"
                f"  Summary: {summary}"
            )
        tasks_block = "\n".join(task_lines) if task_lines else "No tasks saved for this mission."
        return (
            ...
            f"Created At: {mission.get('created_at')}\n"
            f"Saved At: {mission.get('saved_at')}\n\n"
            f"Goal:\n{goal}\n\n"
            f"User Result Preview:\n{user_result}\n\n"
            f"Tasks:\n{tasks_block}"
        )
...
class ApiJobRegistry:
    def __init__(self, queen: "Queen"):
        self.queen = queen
        self.lock = Lock()
        self.jobs: Dict[str, ApiMissionJob] = {}
        self.executor = ThreadPoolExecutor(max_workers=max(1, API_JOB_WORKERS))

    def submit_mission(self, goal: str) -> ApiMissionJob:
        job = ApiMissionJob(goal=goal)
        with self.lock:
            self.jobs[job.id] = job
            self._trim_locked()
        log_api_action(self.queen, "api_job_queued", "API mission job queued.", {"job_id": job.id, "goal_preview": goal[:200]})
        self.executor.submit(self._run_mission_job, job.id)
        return pydantic_deep_copy(job)

    def _run_mission_job(self, job_id: str) -> None:
        with self.lock:
        ...
        log_api_action(self.queen, "api_job_started", "API mission job started.", {"job_id": job_id})
        try:
            # v1.4.2: mission execution stays synchronous inside this worker, but the
            # HTTP request returns immediately. API_JOB_WORKERS defaults to 1 so the
            # Queen does not run multiple full missions concurrently until a future scheduler exists.
            result = self.queen.run_mission(job.goal)
            mission_id = self.queen.last_mission_id
            with self.lock:
                job = self.jobs.get(job_id)
                if job:
            ...
            log_api_action(self.queen, "api_job_completed", "API mission job completed.", {"job_id": job_id, "mission_id": mission_id})
        except Exception as error:
            with self.lock:
                job = self.jobs.get(job_id)
                if job:
                    job.status = "failed"
                    job.error = str(error)
                    job.mission_id = self.queen.last_mission_id
                    job.finished_at = now_utc()
            log_api_action(self.queen, "api_job_failed", "API mission job failed.", {"job_id": job_id, "error": str(error)})

    def list_jobs(self, limit: int = API_DEFAULT_LIMIT) -> List[Dict[str, Any]]:
        with self.lock:
            jobs = sorted(self.jobs.values(), key=lambda item: item.created_at, reverse=True)[:limit]
            return [json_safe_model_dump(job) for job in jobs]

    def get_job(self, job_id: str) -> Optional[Dict[str, Any]]:
        with self.lock:
            job = self.jobs.get(job_id)
            return json_safe_model_dump(job) if job else None

    def _trim_locked(self) -> None:
        if len(self.jobs) <= API_JOB_MAX_HISTORY:
            return
        jobs = sorted(self.jobs.values(), key=lambda item: item.created_at, reverse=True)[:API_JOB_MAX_HISTORY]
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body {
      margin: 0;
      background: radial-gradient(circle at top left, #1a2633 0, var(--bg) 32rem);
      color: var(--text);
    ...
    button { cursor: pointer; background: #172231; }
    button:hover { border-color: var(--accent); }
    .primary { background: #513a16; border-color: #8b6424; }
    .danger { background: #3a1717; border-color: #7c3030; }
    .ok { color: var(--good); }
    .warn { color: var(--warn); }
    .bad { color: var(--bad); }
...
<header>
  <div class="topline">
    <div>
      <h1>ANTHILL CORE v1.5</h1>
      <div class="subtitle">Local UI Foundation · API-driven mission control · Observable swarm backend</div>
    </div>
    <div class="tokenbar">
      <input id="token" type="password" placeholder="ANTHILL API token" />
      <button onclick="saveToken()">Save Token</button>
      <button onclick="refreshAll()">Refresh</button>
      <span id="health" class="pill">health: checking</span>
      <span id="authStatus" class="pill">auth: not checked</span>
    </div>
  </div>
</header>
<main>
  <div class="grid">
    <section class="panel wide">
      ...
      <textarea id="goal" placeholder="Enter a mission for the Queen..."></textarea>
      <div class="row" style="margin-top: 8px;">
        <button class="primary" onclick="submitMission()">Submit Mission</button>
        <span id="missionSubmitStatus" class="mini">Background job mode is enabled.</span>
      </div>
    </section>

    <section class="panel third">
      <h2>System Status</h2>
      <div id="statusPanel" class="mini">Loading...</div>
    </section>

    <section class="panel third">
      <h2>Jobs</h2>
      <div id="jobsPanel" class="mini">Loading...</div>
    </section>

    <section class="panel third">
      <h2>Pheromones</h2>
      <div id="pheromonesPanel" class="mini">Loading...</div>
    </section>

    <section class="panel wide">
      <h2>Live Colony Events</h2>
      <div class="row">
        <input id="eventType" placeholder="event type filter, optional" />
        <input id="eventMission" placeholder="mission id filter, optional" />
        <button onclick="loadEvents()">Filter Events</button>
      </div>
      <div id="eventsPanel">Loading...</div>
    </section>

    <section class="panel">
      <h2>Tasks</h2>
      <div id="tasksPanel">Loading...</div>
    </section>

    <section class="panel">
      <h2>Approvals</h2>
      <div class="row">
        <select id="approvalStatus" onchange="loadApprovals()">
      ...
      <div id="approvalsPanel">Loading...</div>
    </section>

    <section class="panel">
      <h2>Patches</h2>
      <div id="patchesPanel">Loading...</div>
    </section>

    <section class="panel">
      <h2>Sources + Quality</h2>
      <div id="sourcesPanel">Loading...</div>
      <h2 style="margin-top: 16px;">Source Quality</h2>
      <div id="sourceQualityPanel">Loading...</div>
    </section>

    <section class="panel wide">
      <h2>Raw Diagnostics</h2>
      <pre id="diagnosticsPanel">Loading...</pre>
    </section>
  </div>
</main>
<script>
...
function tokenHeaders(extra = {}) {
  const token = localStorage.getItem('anthill_api_token') || '';
  return Object.assign(token ? {'x-anthill-token': token} : {}, extra);
}
async function api(path, options = {}) {
  const opts = Object.assign({}, options);
  opts.headers = tokenHeaders(opts.headers || {});
  const res = await fetch(API + path, opts);
  let data;
  try { data = await res.json(); } catch { data = {ok:false, error:'non-json response'}; }
  if (!res.ok) throw new Error(data.detail || data.error || res.statusText);
  return data;
}
function saveToken() {
  localStorage.setItem('anthill_api_token', document.getElementById('token').value.trim());
  document.getElementById('authStatus').textContent = 'auth: token saved';
  refreshAll();
}
async function checkHealth() {
  try {
    ...
    document.getElementById('health').innerHTML = `<span class="ok">health: online</span> · v${esc(data.data?.version || '?')}`;
  } catch (e) {
    document.getElementById('health').innerHTML = `<span class="bad">health: offline</span>`;
  }
}
async function loadStatus() {
  try {
    const data = await api('/status');
    document.getElementById('authStatus').innerHTML = '<span class="ok">auth: ok</span>';
    const cfg = data.data.config || {};
    document.getElementById('statusPanel').innerHTML = `
      <div class="pill">API ${esc(cfg.api_host)}:${esc(cfg.api_port)}</div>
      <div class="pill">web: ${esc(cfg.web_search_enabled)}</div>
      <div class="pill">writing: ${esc(cfg.file_writing_enabled)}</div>
      <div class="pill">patch apply: ${esc(cfg.patch_application_enabled)}</div>
      <div class="pill">parallel: ${esc(cfg.parallel_execution_enabled)}</div>
      <pre>${esc(data.data.text || '')}</pre>`;
  } catch (e) {
    document.getElementById('authStatus').innerHTML = '<span class="bad">auth: failed</span>';
    document.getElementById('statusPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`;
  }
}
async function submitMission() {
  const goal = document.getElementById('goal').value.trim();
  if (!goal) return;
  document.getElementById('missionSubmitStatus').textContent = 'Submitting mission...';
  try {
    ...
    lastJobId = data.data.job_id;
    document.getElementById('missionSubmitStatus').innerHTML = `Queued job <strong>${esc(lastJobId)}</strong>`;
    startPolling();
    await loadJobs();
  } catch (e) {
    document.getElementById('missionSubmitStatus').innerHTML = `<span class="bad">${esc(e.message)}</span>`;
  }
}
function startPolling() {
  if (pollHandle) clearInterval(pollHandle);
  pollHandle = setInterval(refreshLive, 2500);
}
async function refreshLive() {
  await Promise.allSettled([loadJobs(), loadEvents(), loadTasks(), loadApprovals(), loadPatches(), loadSources(), loadPheromones()]);
}
async function loadJobs() {
  try {
    const data = await api('/api/jobs?limit=10');
    const rows = data.data.rows || [];
    for (const j of rows) if (j.id === lastJobId && j.mission_id) lastMissionId = j.mission_id;
    document.getElementById('jobsPanel').innerHTML = rows.map(j => `
      <div class="card">
        <strong>${esc(j.status)}</strong> · <span class="muted">${esc(j.id)}</span><br/>
        <span class="mini">mission: ${esc(j.mission_id || 'pending')}</span><br/>
        <span class="mini">goal: ${esc((j.goal || '').slice(0,120))}</span>
      </div>`).join('') || '<span class="muted">No jobs yet.</span>';
  } catch (e) { document.getElementById('jobsPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadEvents() {
  try {
    const type = document.getElementById('eventType').value.trim();
    const mission = document.getElementById('eventMission').value.trim();
    const q = new URLSearchParams({limit: '40'});
    if (type) q.set('event_type', type);
    if (mission) q.set('mission_id', mission);
    const data = await api('/events?' + q.toString());
    const rows = data.data.rows || [];
    document.getElementById('eventsPanel').innerHTML = rows.map(e => `
      <div class="card">
        <strong>${esc(e.event_type)}</strong> <span class="muted">${esc(e.created_at)}</span><br/>
        <span class="mini">mission: ${esc(e.mission_id)} · task: ${esc(e.task_id || 'n/a')} · ant: ${esc(e.ant_name || 'n/a')}</span><br/>
        ${esc(e.message || '')}
      </div>`).join('') || '<span class="muted">No events.</span>';
  } catch (e) { document.getElementById('eventsPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadTasks() {
  try {
    const data = await api('/tasks?limit=25');
    const rows = data.data.rows || [];
    document.getElementById('tasksPanel').innerHTML = rows.map(t => `
      <div class="card">
        <strong>${esc(t.status)}</strong> · ${esc(t.title || '')}<br/>
        <span class="mini">ant: ${esc(t.assigned_ant)} · type: ${esc(t.task_type)} · elapsed: ${esc(t.elapsed_seconds ?? 'n/a')}</span><br/>
        <span class="mini">mission: ${esc(t.mission_id)}</span><br/>
        ${esc((t.result_summary || t.result || '').slice(0,220))}
      </div>`).join('') || '<span class="muted">No tasks.</span>';
  } catch (e) { document.getElementById('tasksPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadApprovals() {
  try {
    const status = document.getElementById('approvalStatus').value;
    const data = await api('/approvals?limit=20&status=' + encodeURIComponent(status));
    const rows = data.data.rows || [];
    document.getElementById('approvalsPanel').innerHTML = rows.map(a => `
      <div class="card">
        <strong>${esc(a.status)}</strong> · ${esc(a.action_type || '')}<br/>
        <span class="mini">${esc(a.id)}</span><br/>
        <span class="mini">${esc((a.reason || '').slice(0,220))}</span><br/>
        <button onclick="approve('${esc(a.id)}')">Approve</button>
        <button class="danger" onclick="rejectApproval('${esc(a.id)}')">Reject</button>
        <button onclick="applyApproval('${esc(a.id)}')">Apply</button>
      </div>`).join('') || '<span class="muted">No approvals.</span>';
  } catch (e) { document.getElementById('approvalsPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function approve(id) { await actionPost(`/approvals/${id}/approve`); await loadApprovals(); }
async function rejectApproval(id) {
  const reason = prompt('Reason for rejection?', 'Rejected from ANTHILL UI');
  await actionPost(`/approvals/${id}/reject`, {reason});
  await loadApprovals();
}
async function applyApproval(id) { await actionPost(`/approvals/${id}/apply`); await loadApprovals(); }
async function actionPost(path, body = null) {
  try {
...
async function loadPatches() {
  try {
    const data = await api('/patches?limit=20');
    const rows = data.data.rows || [];
    document.getElementById('patchesPanel').innerHTML = rows.map(p => `
      <div class="card">
        <strong>${esc(p.status)}</strong> · ${esc(p.action || '')} · ${esc(p.target_path || '')}<br/>
        <span class="mini">${esc(p.id)}</span><br/>
        ${esc((p.rationale || '').slice(0,220))}
      </div>`).join('') || '<span class="muted">No patches.</span>';
  } catch (e) { document.getElementById('patchesPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadSources() {
  try {
    const [src, qual] = await Promise.all([api('/sources?limit=12'), api('/source-quality?limit=12')]);
    const rows = src.data.rows || [];
    document.getElementById('sourcesPanel').innerHTML = rows.map(s => `
      <div class="card">
        <strong>${esc(s.confidence_label || 'unknown')}</strong> · ${esc(s.domain || '')}<br/>
        <a href="${esc(s.url)}" target="_blank" rel="noreferrer">${esc((s.title || s.url || '').slice(0,120))}</a><br/>
        <span class="mini">relevance ${esc(s.relevance_score)} · freshness ${esc(s.freshness_score)} · authority ${esc(s.authority_score)}</span>
      </div>`).join('') || '<span class="muted">No sources.</span>';
    const qrows = qual.data.rows || [];
    document.getElementById('sourceQualityPanel').innerHTML = qrows.map(q => `
      <div class="card">
        <strong>${esc(q.domain)}</strong><br/>
        <span class="mini">avg confidence: ${esc(q.avg_confidence_score)} · count: ${esc(q.source_count)}</span>
      </div>`).join('') || '<span class="muted">No source quality data.</span>';
  } catch (e) { document.getElementById('sourcesPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadPheromones() {
  try {
    const data = await api('/pheromones?limit=10');
    const rows = data.data.rows || [];
    document.getElementById('pheromonesPanel').innerHTML = rows.map(p => `
      <div class="card"><strong>${esc(p.trail_key)}</strong><br/><span class="mini">strength ${esc(p.strength)} · success ${esc(p.success_count)} · failure ${esc(p.failure_count)}</span></div>`).join('') || '<span class="muted">No pheromones yet.</span>';
  } catch (e) { document.getElementById('pheromonesPanel').innerHTML = `<span class="bad">${esc(e.message)}</span>`; }
}
async function loadDiagnostics() {
  try {
    const data = await api('/diagnostics');
    document.getElementById('diagnosticsPanel').textContent = data.data.text || '';
  } catch (e) { document.getElementById('diagnosticsPanel').textContent = e.message; }
}
async function refreshAll() {
  await checkHealth();
  await Promise.allSettled([loadStatus(), loadJobs(), loadEvents(), loadTasks(), loadApprovals(), loadPatches(), loadSources(), loadPheromones(), loadDiagnostics()]);
}
window.addEventListener('load', () => {
  document.getElementById('token').value = localStorage.getItem('anthill_api_token') || '';
  refreshAll();
...
def ensure_api_system_mission(queen: "Queen") -> None:
    # API events need a real mission_id because the events schema references missions.
    # SQLite foreign keys are often disabled, but keeping a real reserved mission row
    # makes the event ledger structurally safe.
    existing = queen.memory.get_mission(SYSTEM_API_MISSION_ID)
    if existing:
        return
    system_mission = Mission(
        id=SYSTEM_API_MISSION_ID,
        goal="ANTHILL API system events",
        status=MissionStatus.COMPLETE,
        user_result="Reserved mission row for API/auth/permission/job events.",
        debug_result="System mission created by v1.4.2+ API event safety layer.",
        final_result="Reserved mission row for API system events.",
        best_output_task_id=None,
        success_score=1.0,
    )
    queen.memory.save_mission(system_mission)


def api_event_mission_id(queen: "Queen") -> str:
    ensure_api_system_mission(queen)
    return SYSTEM_API_MISSION_ID
...
def build_auth_dependency(queen: "Queen"):
    def require_api_auth(
        ...
        queen.memory.log_event(
            mission_id=api_event_mission_id(queen),
...
def create_api_app() -> "FastAPI":
    ...
    module_ns = sys.modules.get(__name__).__dict__ if sys.modules.get(__name__) else globals()
    for model in (Event, Mission, Task, SourceRecord, PatchProposal, ApprovalRequest, ApiMissionJob):
        try:
            model.model_rebuild(_types_namespace=module_ns)
        except Exception:
            pass
    ...
    app = FastAPI(
        ...
        description=(
            ...
            "sources, pheromones, patches, approvals, diagnostics, model routing, and the local /ui dashboard."
        ),
    )
    ...
    job_registry = ApiJobRegistry(queen)
    ...
    def api_ui():
        # v1.5 local UI foundation. The dashboard is served without API auth so the
        # browser can load it, but every data/action call still uses the token stored
        # locally in the browser and enforced by the API dependencies.
        return HTMLResponse(ANTHILL_UI_HTML)
    ...
    def api_health():
        return json_ok({
            ...
            "api_job_workers": API_JOB_WORKERS,
    ...
    def api_run_mission(request: MissionRequest):
        ...
        log_api_action(queen, "api_mission_requested", "API mission requested.", {"goal_preview": goal[:200], "goal_chars": len(goal), "background": True})
        job = job_registry.submit_mission(goal)
        return json_ok({"job": json_safe_model_dump(job), "job_id": job.id, "status": job.status}, message="Mission queued. Poll /api/jobs/{job_id} or /events for progress.")

    @app.get("/api/jobs", dependencies=[auth_dependency])
    def api_jobs(limit: int = API_DEFAULT_LIMIT):
        require_api_permission(queen, "read_status", "read API jobs")
        return json_ok({"rows": job_registry.list_jobs(limit=clamp_limit(limit)), "worker_count": API_JOB_WORKERS})

    @app.get("/api/jobs/{job_id}", dependencies=[auth_dependency])
    def api_job_detail(job_id: str):
        require_api_permission(queen, "read_status", "read API job detail")
        row = job_registry.get_job(job_id)
        if not row:
    ...
    @app.get("/missions/{mission_id}", dependencies=[auth_dependency])
    def api_mission_detail(mission_id: str):
        require_api_permission(queen, "read_memory", "read mission detail")
        row = queen.memory.get_mission(mission_id)
        if not row:
            raise HTTPException(status_code=404, detail="Mission not found.")
        tasks = queen.memory.get_tasks_for_mission(mission_id)
        return json_ok({"row": row, "tasks": tasks, "text": queen.format_mission_detail(mission_id)})
    ...
    def api_approvals(limit: int = 20, status: str = "pending"):
        ...
        if status.lower() == "all":
            status_enum = None
        clean_limit = clamp_limit(limit, 20)
        rows = queen.memory.list_approval_requests(status=status_enum, limit=clean_limit)
        return json_ok({"rows": rows, "text": queen.format_approvals(limit=clean_limit, status=status_enum)})
...
def run_api_server():
    ...
    if uvicorn is None:
        ...
        return
    app = create_api_app()
    print(f"[bold yellow]ANTHILL Core v1.5 API/UI online at http://{API_HOST}:{API_PORT}[/bold yellow]")
    print("[dim]Local-only API mode. Use /ui for dashboard and /docs for interactive endpoint docs.[/dim]")
...
def main():
    parser = argparse.ArgumentParser(description="ANTHILL Core v1.4.2")
    ...
    print("[bold yellow]ANTHILL Core v1.4.2 online.[/bold yellow]")
    ...
    print(f"[dim]API backend/UI: ON | Token auth: ON | Local: http://{API_HOST}:{API_PORT}/ui | Start with --api[/dim]")
