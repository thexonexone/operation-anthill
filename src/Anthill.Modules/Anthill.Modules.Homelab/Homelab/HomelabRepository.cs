...
public sealed partial class HomelabRepository : IHomelabRepository, IDisposable
{
    ...
    ///
    /// The homelab has always had its own event stream — the <c>homelab_events</c> table, nineteen
    ...
    /// watching a homelab action run saw nothing until they went looking in a different panel.
    ///
    ...
    /// is what every one of the homelab tests relies on.
    /// </summary>
    ...
    public HomelabRepository(string? dbPath = null)
    {
        ...
        var raw = dbPath ?? HomelabRuntime.Options.DatabasePath;
    ...
    /// <summary>
    /// Record a homelab event durably, then announce it. v3.8.7 adds the second half.
    ///
    ...
    /// <summary>
    /// A homelab event, as the colony sees it.
    ///
    /// The type is PREFIXED rather than passed through. Homelab types are drawn from a different
    ...
    /// panels the first time the two vocabularies happened to agree on a word. <c>homelab_</c> keeps
    /// them tellable apart by inspection, and the original type stays in the metadata for anything
    /// that wants to group by it.
    ///
    /// <c>MissionId</c> is frequently empty here, and that is correct: most homelab events are not
    /// caused by a mission. A drifting inventory belongs to the colony, not to a task.
    /// </summary>
    private static ColonyEvent ToColonyEvent(HomelabEvent evt) => new()
    {
        Id = evt.Id,
        EventType = "homelab_" + evt.EventType,
        MissionId = evt.MissionId,
        Message = evt.Message,
        CreatedAt = AnthillTime.ParseIsoOrNow(evt.CreatedAt),
        Metadata = new Dictionary<string, object?>
        {
            ["homelab_event_type"] = evt.EventType,
    ...
    public void UpsertHealthSchedule(Anthill.Modules.Homelab.Health.HealthCheckSchedule schedule, string changedBy)
    {
    ...
    public IReadOnlyList<Anthill.Modules.Homelab.Health.HealthCheckSchedule> ListHealthSchedules()
    {
        var list = new List<Anthill.Modules.Homelab.Health.HealthCheckSchedule>();
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        ...
        while (r.Read())
        {
            list.Add(new Anthill.Modules.Homelab.Health.HealthCheckSchedule
            {
                Id = r.GetString(0), CheckKind = r.GetString(1), Target = r.GetString(2),
                ServiceId = r.IsDBNull(3) ? "" : r.GetString(3), NodeId = r.IsDBNull(4) ? "" : r.GetString(4),
    ...
    public HomelabInventoryExport ExportInventory() => new()
    {
        ...
        AnthillVersion = HomelabRuntime.Options.ColonyVersion,
