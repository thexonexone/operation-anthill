...
namespace Anthill.Modules.Homelab.Notifications;
...
public sealed class NotificationService
{
    ...
    public static bool Enabled => HomelabRuntime.Options.NotificationsEnabled;
    ...
    {
        if (!string.IsNullOrWhiteSpace(HomelabRuntime.Options.SlackWebhook))
            yield return ("slack", HomelabRuntime.Options.SlackWebhook, "text");
        if (!string.IsNullOrWhiteSpace(HomelabRuntime.Options.DiscordWebhook))
            yield return ("discord", HomelabRuntime.Options.DiscordWebhook, "content");
        if (!string.IsNullOrWhiteSpace(HomelabRuntime.Options.GenericWebhook))
            yield return ("generic", HomelabRuntime.Options.GenericWebhook, "");
    }
        ...
        foreach (var (kind, url, key) in Targets())
        {
            var payload = key.Length > 0
                ...
                : JsonSerializer.Serialize(new Dictionary<string, string?>
                {
                    ...
                    ["target"] = alert.Target, ["source"] = "anthill", ["version"] = HomelabRuntime.Options.ColonyVersion,
